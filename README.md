...
- Summary(요약), Claim(청구항), Examiner(심사관), Prior Art(선행기술조사), Drawing(도면), Specification(발명의 설명) 에이전트가 구조화된 상태를 공유하며 청구항 작성부터 검토·보정까지 연결합니다.
...
- PostgreSQL·pgvector·KIPRIS 유사 특허 검색
- 신규성·진보성 위험도 분석
- 청구항·도면·명세서 통합 보고서
