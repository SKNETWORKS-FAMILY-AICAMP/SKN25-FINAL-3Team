...
from .adapters import get_all_claims_text, get_claim_1_text, get_drawings, get_specification_sections
...
def validate_composer_inputs(state: Dict[str, Any]) -> None:
    try:
        ...
        raise ValueError(f"청구항 1항 오류: {exc}") from exc
    ...
    try:
        ...
        raise ValueError(f"청구항 전체 내용 오류: {exc}") from exc
    ...
    if not any(str(spec_sections.get(key, "")).strip() for key in spec_candidate_keys):
    ...
    if not isinstance(state.get("claims"), dict):
        raise ValueError("claims는 dict 형태의 PatentAgentState 값이어야 합니다.")

    if not isinstance(state.get("specification"), dict):
        raise ValueError("specification은 dict 형태의 PatentAgentState 값이어야 합니다.")
