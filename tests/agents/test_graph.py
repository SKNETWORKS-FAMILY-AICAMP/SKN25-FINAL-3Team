...
def _make_full_mock_adapters() -> dict:
    """6개 agent mock adapter를 모두 만들어 반환합니다."""
    pipeline = [
        ("summary", "summary"),
        ("prior_art", "prior_art"),
        ("claim", "claims"),
...
def test_run_pipeline_with_mock_adapters_status_completed():
    adapters = _make_full_mock_adapters()
    state = run_service_pipeline("충분히 긴 발명 설명입니다. 30자 이상이어야 합니다.", adapters)
    assert state["workflow"]["status"] == "completed"


def test_run_pipeline_with_mock_adapters_all_agents_called():
    adapters = _make_full_mock_adapters()
    run_service_pipeline("충분히 긴 발명 설명입니다. 30자 이상이어야 합니다.", adapters)
    for adapter in adapters.values():
        adapter.run.assert_called_once()


def test_run_pipeline_results_stored_in_state():
    adapters = _make_full_mock_adapters()
    state = run_service_pipeline("충분히 긴 발명 설명입니다. 30자 이상이어야 합니다.", adapters)
...
def test_run_pipeline_partial_route_only_runs_specified_agents():
    adapters = _make_full_mock_adapters()
    run_service_pipeline(
    ...
    adapters["summary"].run.assert_called_once()
    adapters["prior_art"].run.assert_not_called()
    adapters["claim"].run.assert_not_called()


def test_run_pipeline_adapter_validation_error_sets_status_failed():
    adapters = _make_full_mock_adapters()
    adapters["summary"].run.side_effect = AgentValidationError(
        agent_name="summary",
...
def test_run_pipeline_unexpected_exception_sets_status_failed():
    adapters = _make_full_mock_adapters()
    adapters["claim"].run.side_effect = RuntimeError("예상치 못한 오류")
...
def test_run_pipeline_progress_callback_called_for_each_agent():
    adapters = _make_full_mock_adapters()
    callback = MagicMock()
    run_service_pipeline(
        "충분히 긴 발명 설명입니다. 30자 이상이어야 합니다.",
...
def test_run_pipeline_unregistered_adapter_records_error():
    adapters = _make_full_mock_adapters()
    state = run_service_pipeline(
        "충분히 긴 발명 설명입니다. 30자 이상이어야 합니다.",
        adapters,
...
def test_run_pipeline_continues_from_initial_state():
    ...
    # summary adapter는 호출되지 않아야 합니다 (route에 없음)
    adapters["summary"].run.assert_not_called()
    adapters["prior_art"].run.assert_called_once()
