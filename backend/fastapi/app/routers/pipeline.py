...
from backend.fastapi.app.db import REDIS_TTL as _REDIS_TTL, get_db, redis_client as _redis
...
async def run_pipeline(request: PipelineRunRequest, db: Session = Depends(get_db)) -> dict[str, Any]:
    ...
    # 예외로 to_thread가 실패해도 DB 레코드를 항상 업데이트합니다.
    state: dict[str, Any] = {"workflow": {"status": "failed", "errors": [], "trace": []}}
    try:
        ...
        state.setdefault("run_session", {})["run_id"] = run_id
...
async def continue_pipeline(
    ...
    if request.user_input:
        ...
        # wait_user 상태에서 사용자가 추가 입력을 제공한 경우, stale summary를 초기화해서
        # 업데이트된 user_input으로 summary를 재실행합니다.
        # 이렇게 하지 않으면 이전 summary의 clarification_questions가 남아
        # 파이프라인이 wait_user 루프에서 영원히 벗어나지 못합니다.
        if (state.get("workflow") or {}).get("status") == "wait_user":
            state["summary"] = {}
    ...
    # run_id에 해당하는 DB 레코드가 없으면 새로 생성합니다.
    # (state에 run_session.run_id가 없는 경우 새 UUID가 생성되므로 레코드도 만들어야 합니다.)
    db_run = db.query(Run).filter(Run.run_id == run_id).first()
    if db_run is None:
        db_run = _save_run_start(db, run_id, state.get("user_input", ""))
        state.setdefault("run_session", {})["run_id"] = run_id

    next_state: dict[str, Any] = {"workflow": {"status": "failed", "errors": [], "trace": []}}
    try:
