...
from .models import PatentProject, InventionInput, ConsultationState, ChatMessage, PatentClaim, PatentDrawingFile, PriorArtReport, SpecificationDocument
...
def dashboard(request):
    ...
    return render(request, 'workspace/inventor_dashboard.html', {'projects': projects})
...
def generate_drawings_api(request, project_id):
    ...
    try:
        ...
        for dwg in drawing_spec.drawings:
            ...
            PatentDrawingFile.objects.create(project=project, title=dwg.title, image_url=web_url)

        ChatMessage.objects.create(project=project, role='assistant', content=chat_content)

        return JsonResponse({"status": "success", "message": chat_content, "drawings": drawing_urls})
    except Exception as e:
...
def generate_specification_api(request, project_id):
    ...
    try:
        ...
        SpecificationDocument.objects.update_or_create(
            project=project,
            defaults={'markdown_content': md_content}
        )
